...
<style>body{margin:24px;background:#f4f3ef;color:#282724;font:15px system-ui}main{max-width:1180px;margin:auto}.card{display:grid;grid-template-columns:250px minmax(280px,1fr) minmax(290px,1fr);gap:18px;background:#fff;border-radius:10px;padding:16px;margin:14px 0;box-shadow:0 1px 3px #0002}img{width:100%;border:1px solid #ddd}.label{font-size:12px;color:#6d6962;margin:0 0 5px}.meta{color:#6d6962;font-size:13px}.row{display:flex;gap:10px;margin:10px 0;align-items:center}input,select,button{font:inherit;padding:7px}input{flex:1;min-width:0}input[readonly]{background:#f1f0ed;color:#625f58}button{background:#478ed5;color:white;border:0;border-radius:6px;font-weight:600;cursor:pointer}#status{position:sticky;bottom:10px;background:#282724;color:#fff;padding:10px;border-radius:6px;display:inline-block}.empty{background:#fff;padding:18px;border-radius:8px}@media(max-width:850px){.card{grid-template-columns:1fr}}</style>
...
fetch('/api/queue').then(r=>r.json()).then(({items})=>{if(!items.length){cards.innerHTML='<div class=empty>No review items.</div>';status.textContent='Nothing to review';return}items.forEach(item=>{const e=document.createElement('section');e.className='card';e.innerHTML=`<div><p class=label>Board crop</p><img src="/${item.crop}" alt="${item.diagram} board"></div><div><p class=label>Turn-marker context</p><img src="/${item.context}" alt="${item.diagram} context"><p class=meta>Book p. ${item.bookPage} · chapter PDF p. ${item.chapterPDFPage}</p></div><div><h2>Diagram ${item.diagram}</h2><p>${item.reason}</p><label class=row>Parsed FEN <input class=parsed readonly></label><label class=row>Correct FEN <input class=fen placeholder="8/8/8/8/8/8/8/8 w - - 0 1"></label><label class=row>Whose turn <select class=side><option value="">Choose</option><option value=w>White</option><option value=b>Black</option></select></label><label class=row><input type=checkbox class=accepted ${item.accepted?'checked':''}> Accept this correction</label><button>Save</button></div>`;const side=e.querySelector('.side'),fen=e.querySelector('.fen');e.querySelector('.parsed').value=item.parsedFen||'No parsed FEN';side.value=item.sideToMove||'';fen.value=item.piecePlacement?`${item.piecePlacement} ${item.sideToMove||'?'} - - 0 1`:'';e.querySelector('button').onclick=()=>{const parts=fen.value.trim().split(/\\s+/);const placement=parts[0]||'';const inferred=parts[1]==='w'||parts[1]==='b'?parts[1]:'';save({chapter:item.chapter,diagram:item.diagram,accepted:e.querySelector('.accepted').checked,sideToMove:side.value||inferred,piecePlacement:placement})};cards.append(e)})}).catch(()=>status.textContent='Unable to load review queue');
</script>"""
